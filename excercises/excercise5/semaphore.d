...
// wait() låser, notify() slipper
// PROBLEM: race condition mellom Signal(M) og Wait(PS[priority]) hvor programmet henger seg opp i en blokk
// LØSNING: vi sjekker ikke lenger semaforens verdi, men holder styr på det selv inni mutexen. 
    T allocate(int priority){
        mtx.wait();

        if (busy) {
            // Det var her buggen var, tidsrommet mellom notify/signal og wait
            numWaiting[priority]++;     // registrer at vi venter
            mtx.notify();              // slipp mutex

            sems[priority].wait();       // når vi våkner får vi ressursen
        } else {
            busy = true;
            mtx.notify();
        }
    ...
    void deallocate(T v){
        ...
        if(numWaiting[1] > 0){
            numWaiting[1]--;
            sems[1].notify();   // gi til høy prioritet
        }
        else if(numWaiting[0] > 0){
            numWaiting[0]--;
            sems[0].notify();   // gi til lav prioritet
        }
        else {
            busy = false;
            mtx.notify();       // ingen venter, frigjør mutex
        }
